...
class HTMLReporter:
    ...
    def generate(self, df: pd.DataFrame, title: str = "新策略分析") -> Path:
        ...
        for strategy_name, strategy_df in strategy_data.items():
            if not strategy_df.empty:
                detail_path = self._generate_detail_page(
                    strategy_df,
                    self.output_dir,
                    strategy_name,
                    date_str,
                ...
                strategy_file_map[strategy_name] = detail_path.name
    ...
    def _generate_summary(
        ...
        strategy_file_map: Dict[str, str] | None = None,
    ) -> Path:
        """生成主页"""
        strategy_file_map = strategy_file_map or {}
        ...
        strategy_file_map_js = json.dumps(strategy_file_map, ensure_ascii=False)
...
<body>
    <div class="layout">
        ...
        <div class="main">
            <div class="content" id="content">
                {f'<iframe src="{strategy_file_map.get(first_strategy, f"{first_strategy}.html")}"></iframe>' if first_strategy else '<div class="empty">暂无数据</div>'}
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const STRATEGY_FILES = {strategy_file_map_js};
        ...
        function showStrategy(strategy) {{
            ...
            if (availableStrategies.includes(strategy)) {{
                const filename = STRATEGY_FILES[strategy] || (strategy + '.html');
