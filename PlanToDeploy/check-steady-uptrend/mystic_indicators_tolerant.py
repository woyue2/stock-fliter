...
条件说明：
1. mystic_6up1down_in_window_tolerant: 在窗口内，存在连续6有效涨后1真跌的模式
2. mystic_consecutive_6up_tolerant: 最近6天全部是有效连涨（可含小跌）
3. mystic_today_small_drop_after_6up: 今天小跌，且前6天有效连涨（买入机会）
4. mystic_today_is_6th_up_tolerant: 今天是第6天有效涨（等待回调）
"""
...
def mystic_consecutive_6up_tolerant(
    open_series: pd.Series, 
    close_series: pd.Series, 
    small_drop_threshold: float = DEFAULT_SMALL_DROP_THRESHOLD
) -> bool:
    """
    玄学条件（容忍版）：最近6天"有效连续上涨"
    
    与原版 mystic_consecutive_6up 的区别：
        - 原版：严格要求6天全部阳线
        - 新版：允许小跌，只要跌幅 < 阈值
...
def mystic_6up1down_in_window_tolerant(
    open_series: pd.Series, 
    close_series: pd.Series, 
...
def mystic_today_small_drop_after_6up_tolerant(
    open_series: pd.Series, 
    close_series: pd.Series,
    small_drop_threshold: float = DEFAULT_SMALL_DROP_THRESHOLD
) -> bool:
    """
    玄学条件（容忍版）：今天小跌 + 前6天有效连涨
    
    含义：6连涨后的小幅回调（可能是买入机会）
    
    与原版 mystic_today_down_after_6up 的区别：
        - 原版：今天必须是阴线，前6天严格阳线
...
def mystic_today_is_6th_up_tolerant(
    open_series: pd.Series, 
    close_series: pd.Series,
    small_drop_threshold: float = DEFAULT_SMALL_DROP_THRESHOLD
) -> bool:
...
def mystic_6up_prev_down_tolerant(
    open_series: pd.Series, 
    close_series: pd.Series,
    small_drop_threshold: float = DEFAULT_SMALL_DROP_THRESHOLD
) -> bool:
    """
    玄学条件（容忍版）：6有效连涨，且之前那天是真跌
...
def compute_all_mystic_indicators_tolerant(
    open_series: pd.Series, 
    close_series: pd.Series,
    lookback_days: int = 10,
    small_drop_threshold: float = DEFAULT_SMALL_DROP_THRESHOLD
    ...
    """
    return {
        "mystic_6up1down_10d_tolerant": mystic_6up1down_in_window_tolerant(
            open_series, close_series, lookback_days, small_drop_threshold
        ),
        "mystic_consecutive_6up_tolerant": mystic_consecutive_6up_tolerant(
            open_series, close_series, small_drop_threshold
        ),
        "mystic_today_small_drop_after_6up": mystic_today_small_drop_after_6up_tolerant(
            open_series, close_series, small_drop_threshold
        ),
        "mystic_today_is_6th_up_tolerant": mystic_today_is_6th_up_tolerant(
            open_series, close_series, small_drop_threshold
        ),
        "mystic_6up_prev_down_tolerant": mystic_6up_prev_down_tolerant(
            open_series, close_series, small_drop_threshold
        ),
    }
...
MYSTIC_COLUMN_MAP_TOLERANT = {
    "mystic_6up1down_10d_tolerant": "玄学_10天内有6连涨后1跌(容忍小跌)",
    "mystic_consecutive_6up_tolerant": "玄学_最近6天连续涨(容忍小跌)",
    "mystic_today_small_drop_after_6up": "玄学_今天小跌且前6天连涨(买入机会)",
    "mystic_today_is_6th_up_tolerant": "玄学_今天是第6天涨(等待)",
    "mystic_6up_prev_down_tolerant": "玄学_6连涨前一天是真跌",
}

# 反向映射
MYSTIC_COLUMN_MAP_TOLERANT_REVERSE = {v: k for k, v in MYSTIC_COLUMN_MAP_TOLERANT.items()}


def get_mystic_priority_tolerant(row: dict) -> Tuple[int, str]:
    """
    获取玄学优先级（容忍版）
    ...
    if row.get("mystic_today_small_drop_after_6up") or row.get("玄学_今天小跌且前6天连涨(买入机会)"):
        return (1, "[STAR]买入-小回调")
    
    # 优先级2：今天是第6天有效涨 - 等待回调
    if row.get("mystic_today_is_6th_up_tolerant") or row.get("玄学_今天是第6天涨(等待)"):
        return (2, "[STAR]等待-连涨中")
    
    # 优先级3：最近6天有效连涨
    if row.get("mystic_consecutive_6up_tolerant") or row.get("玄学_最近6天连续涨(容忍小跌)"):
        return (3, "◆连涨中")
    
    # 优先级4：10天内有6连涨模式
    if row.get("mystic_6up1down_10d_tolerant") or row.get("玄学_10天内有6连涨后1跌(容忍小跌)"):
        return (4, "○模式")
...
if __name__ == "__main__":
    ...
    result_1 = mystic_consecutive_6up_tolerant(opens_1, closes_1, small_drop_threshold=1.64)
    ...
    result_2 = mystic_consecutive_6up_tolerant(opens_2, closes_2, small_drop_threshold=1.64)
    ...
    result_3 = mystic_consecutive_6up_tolerant(opens_3, closes_3, small_drop_threshold=1.64)
    ...
    result_4 = mystic_consecutive_6up_tolerant(opens_4, closes_4, small_drop_threshold=1.64)
    ...
    indicators = compute_all_mystic_indicators_tolerant(opens_5, closes_5, lookback_days=10, small_drop_threshold=1.64)
    print("  所有指标结果:")
