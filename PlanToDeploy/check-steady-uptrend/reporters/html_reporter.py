...
from utils import infer_market_prefix, get_eastmoney_url
...
class HtmlReporter:
    ...
    def generate(self):
        ...
        # 自动打开总览页面, 可通过 NO_BROWSER=1 禁用
        no_browser = os.environ.get("NO_BROWSER", "").lower() in {"1", "true", "yes", "y"}
        if no_browser:
            print("  [START] NO_BROWSER=1, 跳过自动打开浏览器。")
        else:
            try:
...
<head>
  ...
  <title>股票筛选总览 - {display_date}</title>
  <style>
    ...
    .checked-list li.highlighted {{
      background: #fff9c4;
      border-color: #fbc02d;
    }}
    .checked-list .stock-row {{
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 8px;
    }}
    .checked-list .stock-info {{
      flex: 1;
      display: flex;
      align-items: center;
      flex-wrap: wrap;
    ...
    .checked-list .badge.buy {{
      background: #fce4ec;
      color: #c62828;
    }}
    .checked-list .badge.wait {{
      background: #fff3e0;
      color: #e65100;
    }}
    .checked-list .remove-btn {{
      background: #ffebee;
      border: none;
      color: #c62828;
      cursor: pointer;
    ...
    .checked-list .remove-btn:hover {{
      background: #ffcdd2;
    }}
    /* 备注区域 */
    .checked-list .note-row {{
      margin-top: 6px;
      display: flex;
      gap: 6px;
      align-items: center;
    }}
    .checked-list .note-input {{
      flex: 1;
      padding: 4px 8px;
      border: 1px solid #ddd;
      border-radius: 4px;
    ...
    .checked-list .note-input:focus {{
      outline: none;
      border-color: #1a73e8;
      background: #fff;
    }}
    .checked-list .note-display {{
      font-size: 0.85em;
      color: #555;
      background: #f5f5f5;
      padding: 4px 8px;
    ...
    .checked-list .note-display:hover {{
      background: #e8e8e8;
    }}
    .clear-all-btn {{
      margin-top: 10px;
...
<body>
  ...
  <script>
    ...
    function loadCombo(filename) {{
      document.getElementById('mainFrame').src = filename;
    }}
    ...
    function renderCheckedList() {{
      ...
      list.innerHTML = entries.map(([code, info]) => {{
        let badge = '';
        if (info.priority === '[STAR]买入') {{
          badge = '<span class="badge buy">[STAR]买入</span>';
        }} else if (info.priority === '[STAR]等待') {{
          badge = '<span class="badge wait">[STAR]等待</span>';
        }}
          ...
          <li data-code="${{code}}" class="highlighted">
            <div class="stock-row">
              <div class="stock-info">
                <a onclick="viewStock('${{code}}')">
                  <span class="code">${{code}}</span> ${{info.name || ''}}
                </a>
                ${{badge}}
                ${{industryDisplay}}
    ...
    function saveNoteFromInput(code, input) {{
      saveNote(code, input.value.trim());
      renderCheckedList();
    }}
...
<head>
  ...
  <style>
    ...
    .sidebar .stats-table tr:hover {{
      background: #e1f5fe;
    }}
    .sidebar .stats-table .pct {{
      color: #1565c0;
      font-weight: 500;
    }}
    ...
    .sidebar .stats-content.collapsed {{
      display: none;
    }}
    .sidebar ul {{
      list-style: none;
...
{chr(10).join(list_items)}
  ...
  <script>
    ...
    function updateCheckedCount() {{
      const stocks = getCheckedStocks();
      document.getElementById('checkedCount').textContent = Object.keys(stocks).length;
    }}
    ...
    def _export_to_index_csv(self, df: pd.DataFrame, module_name: str, 
        ...
        # 云端精简版: 索引写入 PlanToDeploy/stocks_index/日期/stocks_index.csv
        index_dir = Path(__file__).parent.parent.parent / "stocks_index" / date_folder
