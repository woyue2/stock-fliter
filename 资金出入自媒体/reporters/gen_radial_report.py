...
[POS]: check-zhulistrength/reporters/gen_radial_report.py, 量化雷达坐标轴终端（V5黑金版）自动化数据引擎
[INPUT]: output/Analyzed_*.csv
...
def gen_radial_report():
    ...
    # 获取项目根目录 (reporters 的上一级)
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    output_dir = os.path.join(base_dir, 'output')
    
    # 1. 找到所有分析结果，按时间排序
    patterns = ['Analyzed_THS_*.csv', 'Analyzed_GN_*.csv', 'Analyzed_HY_*.csv', 'Analyzed_RES_*.csv']
    list_of_files = []
    for p in patterns:
        list_of_files.extend(glob.glob(os.path.join(output_dir, p)))
    ...
    # ① 策略狙击: 错杀 (包含 '恐慌错杀' 或 '错杀')
    snipe_mask = df['主力行为'].str.contains('错杀', na=False)
    snipe_df = df[snipe_mask].sort_values(by='abs_intensity', ascending=False)

    # ② 核心持仓: 真洗盘/吸筹/建仓 (包含 '真洗盘', '真建仓', '暗中吸筹')
    hold_mask = df['主力行为'].str.contains('真洗盘|真建仓|吸筹', na=False)
    hold_df = df[hold_mask].sort_values(by='abs_intensity', ascending=False)

    # ③ 避险雷达: 诱多/假/出货/派发 (包含 '诱多', '假', '真出货', '高位派发')
    trap_mask = df['主力行为'].str.contains('假|诱多|出货|派发', na=False)
    ...
    radial_data = [
        {
            ...
            "items": df_to_items(snipe_df.head(15))
        },
        {
            "title": "主力趋势吸纳",
            "color": "#40A9FF",
            "sentiment": calc_sentiment(hold_df),
            "items": df_to_items(hold_df.head(15))
        },
        {
            "title": "减仓及风险项",
            "color": "#FF4D6D",
            "sentiment": calc_sentiment(trap_df, positive_is_good=False),
            "items": df_to_items(trap_df.head(15))
        }
    ]

    # 5. 读取模板并注入
    template_path = os.path.join(base_dir, 'templates', 'radial_terminal_template.html')
    ...
    # 注入日期标题
    date_match = re.search(r'\d+', os.path.basename(latest_file))
    date_str = date_match.group() if date_match else datetime.now().strftime("%m%d")
    final_html = final_html.replace('{{date}}', date_str)

    # 6. 保存报告
    timestamp = datetime.now().strftime("%m%d_%H%M")
    output_path = os.path.join(output_dir, f'Radial_Report_{date_str}_{timestamp}.html')
