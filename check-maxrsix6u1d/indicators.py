...
def compute_ma_alignment_flags(df: pd.DataFrame, config: MAAlignmentConfig) -> pd.DataFrame:
    df = df.copy()
...
def ma_alignment_signal(df: pd.DataFrame, config: MAAlignmentConfig) -> Dict[str, object]:
    ...
    df_flags = compute_ma_alignment_flags(df, config)
