...
# INPUT:  pd.DataFrame containing stock combos
# OUTPUT: Path to HTML index 
# POS:    check-tdxmacdxvolume/reporters/html_reporter.py
# -*- coding: utf-8 -*-
"""
HTML 报告生成器 - 极简风格，复用 check-td 的格式
...
class HTMLReporter:
    ...
    # 策略类别定义
    STRATEGY_CATEGORIES = {
        "强力买入": "MAxRSIx6U1D AND 放量突破",
        "潜力反转": "NOT MAxRSIx6U1D AND MACD零轴下金叉", 
        "一般持有": "MAxRSIx6U1D AND NOT 放量突破",
    ...
    def generate(self, df: pd.DataFrame, title: str = "TDxMACDxVolume 共振分析") -> Path:
        """生成 HTML 报告"""
        now = datetime.now()
        ...
        # 导出到索引CSV
        self._export_to_index_csv(df, "TDxMACDxVolume", date_str, summary_path)
    ...
    def _generate_summary(self, strategy_data: dict, report_dir: Path, title: str, filename: str = "summary.html", date_str: str = None) -> Path:
        ...
        strategy_group_map = {
            ...
            # 中买入信号（6个）
            "MACD金叉+放量突破": "中买入信号",
            "非MAxRSIx6U1D+MACD金叉": "中买入信号",
            "TD9+MAxRSIx6U1D": "中买入信号",
            "TD7+MACD金叉+阴线": "中买入信号",
            "TD8+MACD金叉+阴线": "中买入信号",
            "TD9+MACD金叉+阴线": "中买入信号",
            # 观察信号（4个）
            "MAxRSIx6U1D+MACD金叉": "观察信号",
            "MAxRSIx6U1D+非放量突破": "观察信号",
            "TD9或MACD金叉": "观察信号",
        ...
        strategy_desc_map = {
            ...
            "MAxRSIx6U1D+放量突破": "MAxRSIx6U1D + 放量突破",
            "TD9+放量突破": "TD9 + 放量突破",
            ...
            "非MAxRSIx6U1D+MACD金叉": "非MAxRSIx6U1D + MACD金叉",
            "TD9+MAxRSIx6U1D": "TD9 + MAxRSIx6U1D",
            "TD7+MACD金叉+阴线": "TD7 + MACD金叉 + 阴线",
            "TD8+MACD金叉+阴线": "TD8 + MACD金叉 + 阴线",
            "TD9+MACD金叉+阴线": "TD9 + MACD金叉 + 阴线",
            # 观察信号
            "MAxRSIx6U1D+MACD金叉": "MAxRSIx6U1D + MACD金叉",
            "MAxRSIx6U1D+非放量突破": "MAxRSIx6U1D + 非放量突破",
...
'''
    ...
    def _write_assets(self):
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sidebar { width: 200px; background: #fff; border-right: 1px solid #e0e0e0; display: flex; flex-direction: column; }
        .logo { padding: 20px; font-size: 16px; font-weight: 600; color: #333; border-bottom: 1px solid #e0e0e0; }
        .nav { flex: 1; overflow-y: auto; padding: 10px 0; }
        .nav-group { padding: 12px 20px 6px; font-size: 12px; color: #ff6b00; font-weight: 700; font-size: 13px; }
        ...
        .strategy-count { font-size: 12px; background: #e0e0e0; padding: 2px 8px; border-radius: 10px; }
        .nav-item.active .strategy-count { background: #1976d2; color: #fff; }
        .nav-item.disabled { color: #ccc; cursor: not-allowed; }
        .nav-item.disabled:hover { background: #fff; }
        .nav-item.disabled .strategy-count { background: #eee; color: #ccc; }
        .selected-stocks { padding: 15px 20px; border-top: 1px solid #e0e0e0; background: #fafafa; max-height: 150px; overflow-y: auto; }
        .selected-title { font-size: 12px; color: #666; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center; }
        .selected-list { display: flex; flex-wrap: wrap; gap: 4px; }
        .selected-item { display: inline-flex; align-items: center; padding: 2px 6px; background: #e3f2fd; border-radius: 3px; font-size: 11px; color: #1976d2; }
        .selected-item .remove { margin-left: 4px; cursor: pointer; color: #999; }
        .selected-item .remove:hover { color: #d32f2f; }
        .notes-section { padding: 15px 20px; border-top: 1px solid #e0e0e0; background: #fff; }
        .notes-label { font-size: 12px; color: #666; margin-bottom: 6px; }
        .notes-input { width: 100%; border: 1px solid #e0e0e0; border-radius: 4px; padding: 8px; font-size: 12px; resize: none; font-family: inherit; }
        ...
        @media (prefers-color-scheme: dark) {
            ...
            .nav-group { color: #999; }
            .notes-section { background: #242424; border-color: #444; color: #e0e0e0; }
            .notes-input { background: #1e1e1e; border-color: #555; color: #e0e0e0; }
            .content { background: #1a1a1a; }
            .selected-stocks { background: #1b1b1b; border-color: #333; }
            .selected-item { background: #1e3a5f; color: #8ab4f8; }
        }
        '''
        ...
        detail_css = '''
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: -apple-system, "Microsoft YaHei", sans-serif; font-size: 13px; color: #333; background: #fff; display: flex; height: 100vh; overflow: hidden; }
        .left-panel { width: 35%; min-width: 400px; display: flex; flex-direction: column; border-right: 1px solid #e0e0e0; }
        ...
        .stock-frame { flex: 1; border: none; width: 100%; }
        ...
        .search { padding: 6px 10px; border: 1px solid #ddd; border-radius: 4px; width: 150px; }
        ...
        .code { color: #1976d2; text-decoration: none; }
        .code:hover { text-decoration: underline; }
        .td { font-weight: 600; text-align: center; }
        .td-high { color: #d32f2f; }
        .td-mid { color: #f57c00; }
        .td-low { color: #999; }
        .macd-strength { text-align: center; font-weight: 600; }
        .detail { font-size: 11px; color: #1976d2; }
        ...
        for _, row in df.iterrows():
            ...
            for col in df.columns:
                if col in ["代码", "名称", "板块", "行业", "最新收盘", "最新日期",
                           "MAxRSIx6U1D", "放量突破", "MACD金叉(零下)", "MACD值", "TD计数", "所属策略"]:
                    continue
                if row.get(col, False) is True:
                    strategies.append(col)
            strategy_str = ", ".join(strategies) if strategies else str(row.get("所属策略", ""))
