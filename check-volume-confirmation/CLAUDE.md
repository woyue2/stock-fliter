# check-volume-confirmation/CLAUDE.md

[PROTOCOL]: 变更时更新此头部，然后检查父级 /CLAUDE.md

## 模块定位

**量价确认筛选**：策略为"昨放量（昨量 > 前3天均量）+ 今阳线"，附带试盘行为识别。
...
- `volume_analyzer.py` 只含策略判断，**不含 IO/网络操作**
- HTML 输出格式（列定义/分组方式）与 check-trend-bottom 保持一致风格
