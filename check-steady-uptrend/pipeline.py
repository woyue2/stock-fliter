...
class Pipeline:
    ...
    def _run_analyzers(self):
        """运行各分析器"""
        # 1. 稳步上升分析
        if self.config.use_steady_uptrend:
            print("\n[1/4] 运行稳步上升分析...")
            self.result.steady_uptrend = self._run_steady_uptrend()
        
        # 2. 趋势分析（包含玄学指标）
        if self.config.use_trend_analysis or self.config.use_mystic:
            print("\n[2/4] 运行趋势分析...")
        ...
        # 4. 玄学指标汇总
        if self.config.use_mystic:
            print("\n[4/4] 汇总玄学指标...")
            self.result.mystic = self._extract_mystic_from_trend()
    
    def _run_steady_uptrend(self) -> AnalysisResult:
        """运行稳步上升分析"""
        from analyzers.steady_uptrend_analyzer import SteadyUptrendAnalyzer

        analyzer = SteadyUptrendAnalyzer(
    ...
    def _extract_mystic_from_trend(self) -> AnalysisResult:
        ...
        mystic_cols = [c for c in df.columns if c.startswith("玄学_")]
    ...
    def _load_existing_results(self):
        ...
        if self.config.use_steady_uptrend:
            ...
            if steady_path:
                ...
                self.result.steady_uptrend = AnalysisResult(
                    name="steady_uptrend",
                    data=df,
                    summary={"total": len(df), "source": str(steady_path)}
                )
                print(f"  [OK] 加载稳步上升: {steady_path.name}")
    ...
    def _combine_results(self):
        ...
        combiner = XuanxueCombiner(
            trend_result=self.result.trend_analysis,
            steady_result=self.result.steady_uptrend,
