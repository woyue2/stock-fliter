...
class TrendAnalyzer:
    ...
    def run(self) -> tuple[pd.DataFrame, Optional[datetime]]:
        ...
        if not result_df.empty:
            result_df = self._rename_mystic_columns(result_df)
        ...
        # 打印数据最新日期
        if global_max_date:
            print(f"[趋势分析] 数据最新日期: {global_max_date.strftime('%Y-%m-%d')}")
    ...
    def _save_results(self, df: pd.DataFrame):
        ...
        if mystic_cols:
            ...
            mystic_path = self.output_dir / f"mystic_candidates_{ts}.csv"
            mystic_df.to_csv(mystic_path, index=False, encoding="utf-8-sig")
            print(f"  [OK] 玄学候选: {mystic_path.name} ({len(mystic_df)} 只)")
