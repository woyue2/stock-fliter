...
## 变动  连板天梯归档数据修复与区域热力映射全量补齐
### 原因  1. 0317 自动抓取归档数据异常导致跨日对比分析失真；2. 连板报告中存在大量“未知”归属地的股票，掩盖了区域联动炒作的真实信号（如粤港澳科技股）。
### 影响  1. 恢复 0317 连板数据，修正历史对比基准。 2. 将 190 只带未知属性的股票注册地批量写入 `scripts/region_map.json`。3. 重刷 `0311`~`0318` 所有历史历史数据，消灭“未知”地区标签。4. 修改 `SKILL.md` 说明文档。

## 变动  新增热门资金追踪脚本与多端数据同步能力
### 原因  1. 需要辅助挖掘当前热点板块中的低位补涨标的；2. `sync_missing_to_db.py` 缺乏双向数据补全支持；3. 需要记录整理更多板块题材的系统化分析与选股心得。
### 影响  1. 新增 `check_hot.py` 和 `query_chuneng.py` 等选股脚本。2. 在 `sync_missing_to_db.py` 中实现了 `sync_bidirectional` 双向同步补齐。3. 新增并追踪了 `所有板块/` 下的各类指南及分析记录文件，丰富个股挖掘基建知识库。

# 2026-03-17

## 变动  底部指标库增强与 EXPMA 指标对齐
### 原因  需与通达信等主流软件的 EXPMA 计算逻辑对齐，为趋势跟踪系统提供底层支持。
### 影响  `util/indicators_lib.py` 新增 `calculate_expma` 方法，采用 `ewm(span=period, adjust=False)` 算法确保数值精确。

## 变动  连板天梯分析逻辑优化与多维度实验验证
### 原因  在高标溢价回归背景下，验证“微笑曲线”与“神奇数字”在极短线博弈中的胜率。
### 影响  更新 `连板天梯/scripts/analyze.py`；新增 `神奇数字/微笑+神奇/` 与 `check-smile-magic/` 工具集；引入 `三线2560三线/` 与 `量阳寻阳/高开篮子/` 概率测试脚本。

## 变动  持仓战法总结与个股操作逻辑实例化
### 原因  强化复盘深度，并针对近期重点标的（动力源、上汽集团）建立独立持仓档案。
### 影响  重构 `持仓逻辑/战法总结`；新增 `持仓逻辑/上汽集团` 与 `持仓逻辑/动力源` ；将实验性低吸策略迁移至 `没用的尝试tn6/`。

## 变动  战略规划文档补齐
### 原因  为项目后续“十五五”阶段的炒作模块演化定调。
### 影响  新增 `docs/十五五计划.md` 与 `docs/十五五炒作模块.md` 战略文档。

# 2026-03-14

## 变动  回测引擎实战化改造、逻辑精确对齐与“右侧确认”模式重构
### 原因  1. 原有回测忽略 T+1 制度和开盘低开风险，导致收益率虚高。
        2. 神奇数字逻辑在两位数股价下存在“跳位”误判。
        3. 左侧摸底 (T0) 实战中容易遇到持续阴跌，需要右侧信号 (T1) 确认。
### 影响  1. **右侧模式**：信号触发点从 T0 延后至 T1（需满足 RSI 勾头+缩量），买入点延至 T2 开盘。实测单笔均益从 0.09% 提升至 **0.39%**。
        2. **实战回测**：引入 T+1 锁定期（买入当天禁止卖出）以及买入次日（T+2）开盘强制止损判定，极大提高了回测的鲁棒性。
        3. **逻辑对齐**：重构 `check_magic`，实现价格区间敏感的数字位提取（D0-D4），修复了 13.07 等个股的误报问题。
        4. **参数增强**：新增 `vol-ratio` 参数，并将默认值从 1.0 微调至 **0.9**（实证显示 0.9 能更好平衡信号过滤与像腾龙股份这种优质标的的捕捉）。
