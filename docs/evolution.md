...
## 变动  回测引擎实战化改造与神奇数字逻辑精确对齐
### 原因  1. 原有回测忽略 T+1 制度和开盘低开风险，导致收益率虚高。
        2. 神奇数字逻辑在两位数股价下存在“跳位”误判。
### 影响  1. **实战回测**：引入 T+1 锁定期（买入当天禁止卖出）以及买入次日（T+2）开盘强制止损判定，极大提高了回测的鲁棒性。
        2. **逻辑对齐**：重构 `check_magic`，实现价格区间敏感的数字位提取（D0-D4），修复了 13.07 等个股的误报问题。
        3. **参数增强**：新增 `vol-ratio` 参数，支持通过比例精确控制“显著缩量”强度。
