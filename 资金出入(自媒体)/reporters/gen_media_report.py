...
def gen_media_report():
    ...
    list_of_files = glob.glob(os.path.join(output_dir, 'Analyzed_*.csv'))
    ...
    # --- 核心过滤逻辑 ---
    win_list = df[df['主力行为'].str.contains('抢筹|建仓|吸筹', na=False) & ~df['主力行为'].str.contains('假|错杀', na=False)].sort_values(by='主力强度', ascending=False).head(5)
    loss_list = df[df['主力行为'].str.contains('出货|假|诱多', na=False) & ~df['主力行为'].str.contains('错杀', na=False)].sort_values(by='主力强度', ascending=True).head(5)
    gold_list = df[df['主力行为'].str.contains('错杀', na=False)].sort_values(by='主力强度', ascending=True).head(5)
    
    # --- 开始生成自媒体文案 ---
    report_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'output', f'Media_Report_{date_str}.md')
